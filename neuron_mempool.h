...
struct neuron_mempool {
	...
	u32 hbm_index; // HBM index valid only if location is device
...
#define MAX_NUM_HBMS 4 // max supported number of HBMs per device, VX_NUM_HBMS macros define it per specific chip 
...
struct neuron_mempool_set {
	...
	u32 num_hbms; // number of HBMs in the device pool
	struct neuron_mempool mp_device[MAX_NUM_HBMS]; // device memory pools
...
struct mem_chunk {
	...
	u32 hbm_index; // HBM index within the device
